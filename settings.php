 ...
 * Strona do zarządzania źródłami danych i testowania
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'change_source' && isset($_POST['data_source'])) {
            ...
            // Test zostanie wykonany poniżej
        }
    }
}

// Pobierz dostępne źródła i ich status
...
$current_source = $user_settings['data_source'] ?? $data_source;

// Pobierz informacje o bieżącym źródle
try {
    $ds = DataSourceFactory::create($current_source);
    $current_info = $ds->getInfo();
} catch (Exception $e) {
    $current_info = ['error' => $e->getMessage()];
}
?>
<!DOCTYPE html>
<html lang="pl">
