                        ...
                        try {
                            $stats = $pdo->query("
                                SELECT 
                                    ...
                                    MIN(kwota) as minimum,
                                    MAX(kwota) as maksimum
                                FROM wydatki
                            ")->fetch();
                            ...
                            echo '<div class="list-group list-group-flush">
                                    <div class="list-group-item d-flex justify-content-between">
                                        ...
                                        <span class="badge bg-primary">' . ($stats['ilosc'] ?? 0) . '</span>
                                    </div>
                                    <div class="list-group-item d-flex justify-content-between">
                                        <span><strong>Suma wydatków:</strong></span>
                                        <span class="badge bg-success">' . number_format($stats['suma'] ?? 0, 2, ',', ' ') . ' zł</span>
                                    </div>
                                    <div class="list-group-item d-flex justify-content-between">
                                        <span><strong>Średnia:</strong></span>
                                        <span class="badge bg-info">' . number_format($stats['srednia'] ?? 0, 2, ',', ' ') . ' zł</span>
                                    </div>
                                    <div class="list-group-item d-flex justify-content-between">
                                        <span><strong>Minimum:</strong></span>
                                        <span class="badge bg-warning">' . number_format($stats['minimum'] ?? 0, 2, ',', ' ') . ' zł</span>
                                    </div>
                                    <div class="list-group-item d-flex justify-content-between">
                                        <span><strong>Maksimum:</strong></span>
                                        <span class="badge bg-danger">' . number_format($stats['maksimum'] ?? 0, 2, ',', ' ') . ' zł</span>
                                    </div>
                                  </div>';
                        } catch (Exception $e) {
            ...
            <div class="col-md-6 mb-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        try {
                            $kategorie = $pdo->query("
                                SELECT kategoria, COUNT(*) as ilosc, SUM(kwota) as suma
                                FROM wydatki
                                GROUP BY kategoria
                                ORDER BY suma DESC
                            ")->fetchAll();
                            ...
                            if (!empty($kategorie)) {
                                ...
                                foreach ($kategorie as $kat) {
                                    echo '<div class="list-group-item d-flex justify-content-between">
                                            <span>
                                                <strong>' . htmlspecialchars($kat['kategoria']) . '</strong>
                                                <br><small class="text-muted">' . $kat['ilosc'] . ' wydatek(i)</small>
                                            </span>
                                            <span class="badge bg-secondary">' . number_format($kat['suma'], 2, ',', ' ') . ' zł</span>
                                          </div>';
                                }
                                echo '</div>';
                            } else {
