    ...
    public function getAll($user_id = null)
    {
        try {
            if ($user_id === null) {
                ...
                $sql = "SELECT * FROM wydatki WHERE user_id = :user_id ORDER BY data_wydatku DESC, created_at DESC";
    ...
    public function getById($id, $user_id = null)
    {
        try {
            if ($user_id === null) {
                ...
                $sql = "SELECT * FROM wydatki WHERE id = :id AND user_id = :user_id";
                $stmt = $this->pdo->prepare($sql);
    ...
    public function add($record, $user_id = null)
    {
        try {
            ...
            if ($user_id === null) {
                ...
                $sql = "INSERT INTO wydatki (user_id, nazwa, kwota, kategoria, data_wydatku) 
                        VALUES (:user_id, :nazwa, :kwota, :kategoria, :data_wydatku)";
                $params = [
                    ':user_id' => (int)$user_id,
                    ':nazwa' => $record['nazwa'],
                    ':kwota' => (float)$record['kwota'],
    ...
    public function delete($id, $user_id = null)
    {
        try {
            if ($user_id === null) {
                ...
                $sql = "DELETE FROM wydatki WHERE id = :id AND user_id = :user_id";
                $stmt = $this->pdo->prepare($sql);
                $stmt->execute([':id' => (int)$id, ':user_id' => (int)$user_id]);
            }
            return $stmt->rowCount() > 0;
